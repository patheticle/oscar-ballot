...
// Helper: draw a filled heart shape centered at (cx, cy) with given size
const drawPDFHeart = (doc, cx, cy, size) => {
  doc.setFillColor(0, 0, 0);
  // Two circles for the top bumps
  doc.circle(cx - size * 0.5, cy - size * 0.25, size * 0.5, 'F');
  doc.circle(cx + size * 0.5, cy - size * 0.25, size * 0.5, 'F');
...
// Generate PDF ballot
// NOTE: Uses CATEGORIES and CATEGORY_ORDER defined below — safe because this
// function is only ever *called* after module-level code has run.
const generateBallotPDF = (name, picks, isBlank = false) => {
  ...
  // ── Name line (with inline legend for filled ballots) ──────────────────────
  doc.setFontSize(10);
  if (isBlank) {
    doc.text('Name: ___________________', marginLeft, 60);
  } else {
    doc.text('Name: ' + name, marginLeft, 60);
    // Legend: ♥ = Want to Win — positioned right side of name line
    doc.setFontSize(8);
    const legendX = pageWidth - marginRight - 72;
    drawPDFHeart(doc, legendX, 57, 3);
    doc.text('= Want to Win', legendX + 7, 60);
  }
  ...
  colCategories.forEach((colCats, colIndex) => {
    ...
    colCats.forEach((category) => {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(9);
      doc.text(category.replace('Best ', ''), x, y);
      y += 13;

      // Nominees — use CATEGORIES directly so picks match exactly
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(8);
      ...
      nominees.forEach((nominee) => {
        ...
        const wantWin = !isBlank && userPick.wantWin === nominee;

        // Radio circle (filled = will win, outline = not picked)
        const circleX = x + 4;
        const circleY = y - 2.5;
        const radius = 3.5;
        if (willWin) {
        ...
        // Heart to the right of the text for want-to-win
        if (wantWin) {
          const textWidth = doc.getTextWidth(display);
          drawPDFHeart(doc, x + 16 + textWidth + 3, y - 2, 3);
        }
  ...
  const filename = isBlank
    ? 'oscar-ballot-blank.pdf'
    : `oscar-ballot-${name.toLowerCase().replace(/\s+/g, '-')}.pdf`;
  doc.save(filename);
};
...
function NomineeRow({ nominee, currentPicks, onPick }) {
  ...
  return (
    <div className="flex items-center gap-2">
      {/* Will Win tap target */}
      <div 
        onClick={() => onPick('willWin', isWillWin ? null : nominee)}
        className={`flex-1 flex items-center px-4 py-3 rounded-xl cursor-pointer transition-all ${
          isWillWin 
      ...
      {/* Want to Win tap target — separate */}
      <button
        onClick={() => onPick('wantWin', isWishWin ? null : nominee)}
        className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center transition-all text-lg border-2 ${
          isWishWin
            ? 'bg-rose-100 border-rose-300 text-rose-500'
            : 'bg-amber-50/50 border-transparent text-amber-300 hover:text-rose-400 hover:bg-rose-50 hover:border-rose-200'
        }`}
      >
        {isWishWin ? '♥' : '♡'}
      </button>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50 flex flex-col items-center justify-center p-6">
        <div className="text-center max-w-md w-full">
          ...
          <p className="text-sm font-semibold text-amber-700 mb-8">Make your picks, share with friends, and</p><p className="text-sm font-semibold text-amber-700 mb-8">score each other's as you watch!</p>
                ...
                {isExpanded && (
                  <div className="px-5 pb-5 space-y-2">
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-xs text-amber-700 font-medium">Who will win</p>
                      <p className="text-xs text-amber-700 font-medium">♡ Want to win</p>
                    </div>
